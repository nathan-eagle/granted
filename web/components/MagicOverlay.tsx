...
export default function MagicOverlay({ projectId, onClose }: { projectId: string; onClose: () => void }){
  ...
  const [live, setLive] = useState<Record<string, { title: string; text: string }>>({})
  ...
  return (
    <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000}}>
      <div style={{width:860, maxWidth:'92vw', background:'#111318', border:'1px solid #1f2430', borderRadius:14, padding:20, color:'#E5E7EB', boxShadow:'0 12px 24px rgba(0,0,0,0.35)'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'280px 1fr', gap:16}}>
          <div>
            ...
            {!!toc.length && (
              <div style={{marginTop:12}}>
                <div style={{fontWeight:600, marginBottom:6}}>Writing now</div>
                <ul style={{listStyle:'none', padding:0, margin:0}}>
                  {toc.map(t => (
                    <li key={t.key} onClick={()=> setActiveKey(t.key)} style={{cursor:'pointer', padding:'4px 6px', borderRadius:6, background: activeKey===t.key ? '#1f2430' : 'transparent', color:'#D1D5DB'}}>
                      {t.title}
                    </li>
                  ))}
                </ul>
          ...
          <div style={{maxHeight:'52vh', overflow:'auto', padding:'8px 12px', border:'1px solid #1f2430', borderRadius:10}}>
            {activeKey && live[activeKey]?.text ? (
              <div>
                <div style={{fontWeight:700, marginBottom:6}}>{live[activeKey]?.title}</div>
                <pre style={{whiteSpace:'pre-wrap', lineHeight:1.5, fontFamily:'system-ui, -apple-system, Segoe UI, Roboto, Arial'}}>{live[activeKey]?.text}</pre>
              </div>
            ) : (
              <div style={{color:'#9CA3AF'}}>Live text will appear here as the section is written…</div>
            )}
          </div>
        </div>
      </div>
