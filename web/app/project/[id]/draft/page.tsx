    ...
    <div style={{display:'grid',gridTemplateColumns:'260px 1fr 300px',gap:24}}>
      <aside style={{borderRight:'1px solid #eee',paddingRight:16}}>
        ...
        <div style={{marginTop:12}}>
          <DocumentsPanel projectId={project.id} uploads={project.uploads.map(u => ({ id: u.id, filename: u.filename, kind: u.kind, url: u.url || '' })) as any} sections={project.sections.map(s => ({ id: s.id, key: s.key, title: s.title })) as any} />
        </div>
        <div style={{marginTop:12}}>
